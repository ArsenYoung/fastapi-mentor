...
class StudentsCoursesService():
    ...
    async def del_student_with_courses(self, student_id: int) -> None:
        ...
        await self.courses_repo.delete(student_id=student_id)
    ...
    async def update_student_with_courses(self, student_id, data: StudentPatch) -> None:
        ...
        if data.course:
            if data.course.number is not None:
                await self._check_course_exist(number=data.course.number)
            course_data = data.course.model_dump(exclude_unset=True)
            await self.courses_repo.update(course_data, student_id=student_id)
