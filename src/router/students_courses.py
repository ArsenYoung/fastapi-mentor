...
from src.schemas.students import Student, StudentCreate, StudentsPaginatedList, StudentUpdate
...
@router.get("{student_id}", summary="Get a student and their courses", response_model=Student, status_code=status.HTTP_200_OK)
...
@router.get("", summary="Get students and their courses", response_model=StudentsPaginatedList, status_code=status.HTTP_200_OK)
...
async def delete_student_with_courses(
    student_id: int,
    service: StudentsCoursesService = Depends(get_students_courses_service)
) -> None:
